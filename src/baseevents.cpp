...
{
	if (loaded) {
		Logger::error() << "[Error - BaseEvents::loadFromXml] It's already loaded." << std::endl;
		return false;
	}
	...
	if (getScriptInterface().loadFile(basePath + "lib/" + scriptsName + ".lua") == -1) {
		Logger::warn() << "[Warning - BaseEvents::loadFromXml] Can not load " << scriptsName << " lib/" << scriptsName << ".lua" << std::endl;
	}
	...
	for (auto node : doc.child(scriptsName.c_str()).children()) {
		...
		if (!event->configureEvent(node)) {
			Logger::error() << "[Warning - BaseEvents::loadFromXml] Failed to configure event" << std::endl;
			delete event;
			continue;
		}
...
bool Event::checkScript(const std::string& basePath, const std::string& scriptsName, const std::string& scriptFile) const
{
	...
	if (testInterface->loadFile(std::string(basePath + "lib/" + scriptsName + ".lua")) == -1) {
		Logger::warn() << "[Warning - Event::checkScript] Can not load " << scriptsName << " lib/" << scriptsName << ".lua" << std::endl;
	}

	if (scriptId != 0) {
		Logger::info() << "[Failure - Event::checkScript] scriptid = " << scriptId << std::endl;
		return false;
	}

	if (testInterface->loadFile(basePath + scriptFile) == -1) {
		Logger::warn() << "[Warning - Event::checkScript] Can not load script: " << scriptFile << std::endl;
		Logger::info() << testInterface->getLastLuaError() << std::endl;
		return false;
	}

	int32_t id = testInterface->getEvent(getScriptEventName());
	if (id == -1) {
		Logger::warn() << "[Warning - Event::checkScript] Event " << getScriptEventName() << " not found. " << scriptFile << std::endl;
		return false;
	}
	return true;
}
...
bool Event::loadScript(const std::string& scriptFile)
{
	if (!scriptInterface || scriptId != 0) {
		Logger::info() << "Failure: [Event::loadScript] scriptInterface == nullptr. scriptid = " << scriptId << std::endl;
		return false;
	}

	if (scriptInterface->loadFile(scriptFile) == -1) {
		Logger::warn() << "[Warning - Event::loadScript] Can not load script. " << scriptFile << std::endl;
	...
	if (id == -1) {
		Logger::warn() << "[Warning - Event::loadScript] Event " << getScriptEventName() << " not found. " << scriptFile << std::endl;
		return false;
	}
...
{
	if (!interface) {
		Logger::info() << "Failure: [CallBack::loadCallBack] scriptInterface == nullptr" << std::endl;
		return false;
	}
	...
	if (id == -1) {
		Logger::warn() << "[Warning - CallBack::loadCallBack] Event " << name << " not found." << std::endl;
		return false;
	}
